...
angular.module('Mipoz')
  .factory('CourseStore', function($http, $timeout, pouchdb, md5, $rootScope, $location) {
    ...
    var CourseStore = {
        ...
        validate: function(encodedData) {
          ...
          if ((hash) && (jsonStr)) {
            ...
            if (hash == md5.createHash(jsonStr)) {} else
              return 'failed';
          } else
            return 'failed';
          return 'success';
        ...
        paramsToFilter: function(encodedData) {
          ...
          for (var code in rawFilter) {
            ...
            courseFilter[code] = {
              indexDict: {},
              // sfSelectedDict: {},
              selected: rawFilter[code].selected.length > 0 ? [rawFilter[code].selected] : [],
              examDate: rawFilter[code].examDate.length > 0 ?
                rawFilter[code].examDate : "None"
            };
    ...
    $rootScope.$watch(function() {
      ...
      // else {
      CourseStore.updateAll(courseFilter);
      $rootScope.$emit('CourseStore.filterChange', {
